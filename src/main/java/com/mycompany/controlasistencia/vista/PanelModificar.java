...
public class PanelModificar extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        jPanel1.add(btnConfirmar, new org.netbeans.lib.awtextra.AbsoluteConstraints(290, 350, 130, 30));
        ...
        jButton1.setText("Quitar contraseña");
        jButton1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton1ActionPerformed(evt);
            }
        });
        jPanel1.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(430, 350, 130, 30));
...
private void quitarContrasena() {
    Integer  id = getIdSeleccionado();
    
    if (id == null) {
        JOptionPane.showMessageDialog(this, "Seleccioné a un usuario en la tabla","Atención", JOptionPane.WARNING_MESSAGE);
        return;
    }
    
    int resp = JOptionPane.showConfirmDialog(this, "¿Seguro de quitar la contraseña?\n" + 
            "Solo podrá inciar sesion si se le asigna una", "Confirmar", 
            JOptionPane.YES_NO_OPTION,JOptionPane.WARNING_MESSAGE);
    if (resp != JOptionPane.YES_NO_OPTION)return;
    
    Integer actorId = (usuario != null) ? usuario.getId_Usuario() : null;
    
    boolean ok = (actorId != null)
            ? usuarioDAO.borrarContraConLog(actorId, id)
            : usuarioDAO.borrarContra(id);
    
    if (ok) {
        JOptionPane.showMessageDialog(this, "Contraseña eliminada");
        
        int idReselect = id;
        cargarUsuariosEnTabla();
        reseleccionarPorId(idReselect);
        txtContra.setText("");
        txtRepetirContra.setText("");
    } else {
        JOptionPane.showMessageDialog(this, "No se pudo eliminar la contraseña", "error", JOptionPane.ERROR_MESSAGE);
    }
}
