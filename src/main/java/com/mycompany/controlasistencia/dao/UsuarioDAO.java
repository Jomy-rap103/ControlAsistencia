...
public class UsuarioDAO {
    ...
    // Validar nombre y comparar con el bcrypt (contraseña hasheada) ahora tiene tambien el rut
   public Usuario iniciarSesion(String rutIngresado, String contrasenaIngresada) {
    String sql = """
        SELECT id_usuario, nombre, apellido, rut, contrasena, activo
        FROM usuarios
        WHERE activo = TRUE
        AND replace(replace(replace(upper(trim(rut)),'.',''),'-',''),' ','') = ?
        LIMIT 1
    """;

    if (rutIngresado == null || rutIngresado.isBlank()) return null;
    String rutNorm = normalizaRut(rutIngresado);

    try (Connection con = ConexionDB.conectar();
         PreparedStatement ps = con.prepareStatement(sql)) {

        ps.setString(1, rutNorm);

        try (ResultSet rs = ps.executeQuery()) {
            if (!rs.next()) return null;

            String hash = rs.getString("contrasena");
            if (!Seguridad.verificar(contrasenaIngresada, hash)) return null;

            boolean activo = rs.getBoolean("activo");
            if(!activo) return null;
            
            int id = rs.getInt("id_usuario");
            String rutDb = rs.getString("rut");
            

            Usuario u = new Usuario(id, rutDb, activo);
            return u;
...
public Usuario buscarPorRut(String rut) {
 ...
 public boolean borrarContra(int idUsuario) {
    final String sql = "UPDATE usuarios SET contrasena = NULL WHERE id_usuario = ?";
    try (Connection con = ConexionDB.conectar();
         PreparedStatement ps = con.prepareStatement(sql)) {
  ...
  public boolean borrarContraConLog(int actorId, int idUsuario) {
    boolean ok = borrarContra(idUsuario);
    if (ok) new HistorialDAO().log(actorId, "EDITAR_USUARIO", "USUARIO", idUsuario, "password: eliminada");
    return ok;
}
